...
const updateEmployeeRole = () => {
    ...
    db.query(`SELECT * FROM employee`,function (err, results) {
        ...
        return inquirer.prompt([
            {
                    type: 'list',
                    messaage: "which employee do you want to update?",
                    name: 'employee',
                    choices: employeeArray
                },
                {
                    type: 'list',
                    messaage: "what is the employee's new role?",
                    name: 'role',
                    choices: roleArray
                },
        ]).then((data) => {
            db.query(`SELECT id FROM role WHERE role.title = ?;`, data.role, (err, results) => {
                role_id = results[0].id;
                db.query(`SELECT id FROM role WHERE employee.first_name = ? AND employee.last_name = ?;`, data.employee.split(""), (err, results) => {
